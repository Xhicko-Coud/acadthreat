...
## 6.1 Auth User And App Profile Separation Rule

Better Auth identity records and AcadThreat application profile records must stay separate.

Rules:

1. Better Auth user creation must receive auth-safe identity fields only.
2. Application role, status, and permissions must be stored in AcadThreat profile/user records, not passed into Better Auth adapter user data.
...
## 6.2 User Role Management Rule

AcadThreat role management controls application authorization only.

Rules:

1. Role changes must update AcadThreat app profile/role records, not Better Auth identity data.
...
## 10.1.2 Admin Operation Wrapper Rule

UI-triggered admin operations must use authenticated role-based backend wrappers.

Rules:

1. Frontend must never send `seedKey`.
2. Frontend must never receive provider secrets.
3. Feed sync UI operations are admin-only in V1.
4. Unsupported providers must return safe `provider_not_enabled` responses.
5. Operation responses must return safe statuses and counts only.
6. Raw provider responses, headers, secrets, stack traces, HMAC details, and backend internals must not be returned.
7. Maintenance seed-key runners may remain for developer operations, but UI must use role-authenticated wrappers.
8. Feed sync UI must call role-authenticated operation wrappers, not seed-key maintenance runners.
9. Feed sync UI must not include API-key forms.
10. Unsupported providers must be marked as coming soon or not enabled.
11. Feed sync operation results must be shown through global notifications, not embedded `AppAlert`.
12. Feed sync UI must not expose raw provider responses, secrets, headers, stack traces, HMAC details, or backend internals.
13. Provider-ready operation UI should use generic selected-provider copy.
14. UI copy must not imply unsupported providers are active.
...
## 10.4.1 Threat Event Admin Operation Rule

UI-triggered threat event analysis operations must use authenticated role-based backend wrappers.

Rules:

1. UI-triggered correlation and scoring must use authenticated role-based backend wrappers.
2. Frontend must never send `seedKey`.
3. Correlation operation must create threat events only through existing backend correlation logic.
4. Scoring operation must update score and priority only through existing backend scoring logic.
5. Operation responses must return safe counts only.
6. Viewers cannot run analysis operations.
7. Raw logs, raw payloads, stack traces, secrets, and backend internals must not be returned.
8. Confirmation dialogs are required before frontend triggers these operations.
9. Maintenance seed-key runners may remain for developer operations, but UI must use role-authenticated wrappers.
10. Threat Events UI must use a single `Run analysis` dropdown for backend analysis operations.
11. Frontend must not call seed-key maintenance runners.
12. Analysis operation results must be shown through global notifications only.
13. Do not add row-level correlation or scoring actions.
...
7. Demo proof seed UI must be clearly labeled as demo/testing.
8. Demo proof seed UI belongs on the Logs page if exposed in the admin UI.
9. Demo proof seed UI may be provider-ready, but only implemented providers may run.
10. Unsupported proof providers must be marked coming soon or not enabled.
11. Demo proof seed UI copy should be provider-generic and use selected provider labels dynamically.
12. Demo proof seed UI must use authenticated role-based wrappers, not seedKey.
13. Demo proof seed UI must not create threatEvents directly.
14. Demo proof seed UI must use the existing ingestion pipeline.
15. Production log ingestion remains the HMAC-protected external system flow.
16. Demo proof seed may support bounded batch seeding for presentation.
17. Batch proof seeding max is 50 in MVP.
18. Each proof log must use an existing imported indicator.
